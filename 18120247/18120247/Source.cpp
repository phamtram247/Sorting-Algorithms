...
{
	int diemTru = mang[phai];

	int i = (trai - 1);

	for (int j = trai; j <= phai - 1; j++)
	{
		if (mang[j] < diemTru)
		{
			i++;
...
void sapXepNhanh(int a[], int bTrai, int bPhai)
{
		if (bTrai <= bPhai)
		{
			int moc = a[(bTrai + bPhai) / 2];
			int i = bTrai;
			int j = bPhai;
			...
			if (bTrai < j)
				sapXepNhanh(a, bTrai, j);
			if (bPhai > i)
				sapXepNhanh(a, i, bPhai);
		}
}
